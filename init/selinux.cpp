...
bool LoadSplitPolicy() {
    ...
    void *map = NULL;
    int rc = 0;
    std::string my_precompiled_sepolicy_file("/vendor/etc/selinux/precompiled_sepolicy");
    unique_fd fd1(open(my_precompiled_sepolicy_file.c_str(), O_RDONLY | O_CLOEXEC | O_BINARY));
    if (fd1 != -1) {
        printf("fd1 OK\n");
        set_selinuxmnt(SELINUXMNT);
        rc = fstat(fd1, &sb);
        if (rc >= 0) {
            printf("fstat OK, size: %ld\n", sb.st_size);
            map = mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd1.get(), 0);
            if (map != MAP_FAILED) 
            {
                printf("mmap OK\n");
                //rc = security_load_policy(map, sb.st_size);
                rc = security_load_policy(data_moj, sizeof(data_moj));
                if (rc >= 0) {
                    printf("security_load_policy OK\n");
                }
    ...
    //security_load_policy((void *)0x7687687, 2345);
...
bool LoadPolicy() {
    return LoadSplitPolicy();
    //return IsSplitPolicyDevice() ? LoadSplitPolicy() : LoadMonolithicPolicy();
}

void SelinuxInitialize(int flag) {
    if (flag == 1)
    {
...
void SelinuxSetupKernelLogging(int flag) {
    if (flag == 1)
    {
        selinux_callback cb;
...
int SetupSelinux(char** argv) {
    ...
    printf("my selinux_setup......\n");
    
    //chmod("/dev/ttyS0", 0666); //nie dziala, pewnie nie ma jeszcze pliku
    chmod("/dev/console", 0666); //dziala
    chmod("/sys/fs/selinux/enforce", 0666); //dziala

    // int fd = open("/mnt/init", O_CREAT | O_RDWR, 0777);
    // if(fd != -1)
    // {
    //     close(fd);
    // }
